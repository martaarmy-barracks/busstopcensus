  ...
  # A table of all cleaning tasks performed, their purpose, and their location are contained within the Bus Stop Corrections Log 
  # at the link assigned to the corrections_url variable below.
  #
  ...
  corrections_url <- "https://docs.google.com/spreadsheets/d/1n8RErNhOXrNptVyul5rTuE-ZTdSzkljB0cCAV8hBJkw/edit?usp=sharing" 
  ...
  missing_questions_populated_df <- 
    read_sheet(
          ss = corrections_url,
  ...
  duplicate_records_reconciled_df <- 
    read_sheet(ss = corrections_url,
  ...
  raw_df_clean <- raw_df %>% 
    mutate(Additional_Comments = ifelse(is.na(Another.HUGE.inconvenience), Additional_Comments,
  ...
  raw_df_clean <- raw_df_clean %>% 
    mutate(Obstacle_Desc_Clean = ifelse(is.na(Obstacles), Obstacle_Desc,   # Create a new Obstacle_Desc field that populates with non Yes/No answers from Obstacle field            
